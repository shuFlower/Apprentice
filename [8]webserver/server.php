...
function getResponse($http_line, $http_header, $http_data){
    ...
    if($method == 'GET')
    {
        ...
        $client_ssid = $param_arr['Cookie'];
        $response_data = dealPhp($http_data, $client_ssid);
    }
    ...
    //2. header
    $length = mb_strlen($response_data, 'utf-8');
...
function dealPhp($command, $client_ssid){

    if(!empty($client_ssid))
    {
        //根据ssid获取密码
        $password = '123456';
        $ssid = $client_ssid;

    }elseif (empty($client_ssid) && $command == 'flower') {
